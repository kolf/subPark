<template>
    <div class="agile" :class="{'agile--fade': fade_}">
        <div class="agile__list">
            <div class="agile__track" :style="{width: width.track + 'px', transform: 'translate(-' + transform + 'px)', transition: 'transform ' + timing_ + ' ' + transitionDelay + 'ms'}">
                <slot></slot>
            </div>

            <ul v-if="dots_" class="agile__dots">
                <li v-for="n in slidesCount" class="agile__dot" :class="{'agile__dot--current': n - 1 === currentSlide}">
                    <i @click="setSlide(n - 1)">{{n}}</i>
                </li>
            </ul>

            <button v-if="arrows_" class="agile__arrow agile__arrow--prev" :disabled="currentSlide === 0 && !infinite_" @click="prevSlide" v-html="arrow_">
            </button>
            <button v-if="arrows_" class="agile__arrow agile__arrow--next" :disabled="currentSlide === slidesCount - 1 && !infinite_" @click="nextSlide" v-html="arrow_">
            </button>
        </div>
    </div>
</template>
